...
pub struct TextInputDataInner {
    ...
    /// The preedit that was already delivered to (and is being displayed by)
    /// the client on the last `done`. `pending_preedit` only lives between
    /// `preedit_string` and `done`, so by the time keyboard focus leaves the
    /// surface it is always `None` — this field is what still knows the
    /// half-composed text that must be committed on focus-out.
    live_preedit: Option<String>,
}
...
impl<T> Dispatch<zwp_text_input_v3::ZwpTextInputV3, TextInputData> for WindowState<T> {
    fn event(
        ...
        match event {
            ...
            Event::Leave { surface } => {
                let id = state.get_id_from_surface(&surface);

                // Flush any in-flight IME composition before tearing down the
                // text-input focus. Without this, the last syllable of a
                // composed (e.g. Hangul) input is silently dropped when the
                // surface loses keyboard focus, because the compositor's
                // deactivate never gives us a chance to commit it. This
                // mirrors GTK's focus-out commit behavior.
                if let Some(id) = id {
                    ...
                    // The composition the client is currently displaying: this
                    // is the field that is actually populated at focus-out time
                    // (`pending_preedit` is drained on every `done`), so this
                    // is the flush that saves the last half-composed syllable.
                    let leftover = text_input_data
                        .pending_preedit
                        .take()
                        .map(|preedit| preedit.text)
                        .or_else(|| text_input_data.live_preedit.take())
            ...
            Event::Done { .. } => {
                ...
                {
                    text_input_data.live_preedit = None;
                    state.message.push((
                        Some(id),
                        DispatchMessageInner::Ime(Ime::Preedit(String::new(), None)),
                    ));
                ...
                if let Some(preedit) = text_input_data.pending_preedit.take() {
                    ...
                    text_input_data.live_preedit = if preedit.text.is_empty() {
                        None
                    } else {
                        Some(preedit.text.clone())
                    };
                    state.message.push((
                        Some(id),
