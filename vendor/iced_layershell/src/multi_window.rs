    ...
    fn handle_ui_state(
        ...
        match ui_state {
            ...
            user_interface::State::Updated {
                ...
                if update_ime {
                    if ev.current_surface_id() == Some(window.id) {
                            ...
                            iced_core::InputMethod::Enabled {
                                ...
                                if ime_flags.contains(ImeState::Update) {
                                    ...
                                    ev.set_ime_cursor_area(
                                        layershellev::dpi::LogicalPosition::new(
                                            cursor.x, cursor.y,
                                        ),
                        ...
                        // Unfocused sibling windows must not clobber the global IME
                        // state the focused window just set. Still reset this
                        // window's local IME state machine so a later refocus
                        // re-fires ImeState::Allowed.
                        let _ = window.request_input_method(iced_core::InputMethod::Disabled);
                    }
                }
